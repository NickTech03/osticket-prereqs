...
(You can use a different name, this is the name I choose that's related to this lab.)

- Region: `(Canda) Canada Central`

(I chose this region as this is closest to me, please choose a region that is local to you.)
...
(I recommend to choose one that has 2 vcpus & 8 GiB of memory but if you need more power, go with the one I selected.)

- Username: `labuser`

(You can change this if you want, whatever is easy to remember.)
...
- Click: `The check box to confirm licensing`
- Click: `Review + create`
...
**IIS (Internet Information Services):** IIS is a web server made by Microsoft that allows a computer to host websites and web applications. In the osTicket setup, IIS is used to run the osTicket website so users can access the helpdesk through a web browser.

**CGI (Common Gateway Interface):** CGI allows the web server (IIS) to run programs like PHP. In osTicket, it helps IIS process PHP files so the website can work properly.
...
**PHP Manager for IIS (PHPManagerForIIS_V1.5.0):** PHP Manager is a tool that helps IIS work properly with PHP. It makes it easier to install, configure, and check PHP settings so that osTicket can run without errors on the web server.
...
The installation for **IIS URL Rewrite Module (rewrite_amd64_en-US)** is complete, you can click `Finish`.
...
**Rewrite Module (rewrite_amd64_en-US):** The Rewrite Module is a tool for IIS that allows you to create clean and properly formatted web addresses (URLs). For osTicket, it helps the website load and work correctly by managing how web links are handled on the server.
...
- Right click within the `Windows C:\ drive` and hover over `New`, then click `Folder`.
...
- A prompt will pop up and for the destination, you can type `C:\PHP` or click browse and navigate to `Windows (C:)` drive were the `PHP` folder is located and select the folder.
...
**PHP (php-7.3.8-nts-Win32-VC15-x86):** This is a specific version of PHP, which is the programming language that osTicket runs on. Installing this allows the web server (IIS) to understand and run osTicket properly on a Windows system.
...
The installation for **VC_redist.x86 (Microsoft Visual C++ Redistributable (x86))** is complete, you can click `Close`.
...
**VC_redist.x86 (Microsoft Visual C++ Redistributable (x86)):** This is a package of files that many Windows programs need to run properly. For osTicket, it provides important background components that allow PHP and other tools to work correctly on your system.
